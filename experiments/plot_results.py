...
if __name__ == "__main__":
    ...
    methods = ("esp_tables_no_offset", "esp_subtables_full", "mtrie")
    method_titles = ("Espresso", "Order-\nExploiting\nEspresso", "m-Trie")
    ...
    for model, ax, title in zip(nets, axs, net_titles):
        ...
        data = np.zeros((len(methods), 144))

        # Read all the data
        for j, method in enumerate(methods):
            fn = "compressed/{}_{}_12_12_xyp.bin".format(method, model)
            with open(fn, "rb") as f:
                data[j, :] = np.array(list(read_table_lengths(f).values()))

        elems = ax.violinplot(data.T, np.arange(len(methods))*2, showextrema=False)
        for b in elems["bodies"]:
            b.set_alpha(.75)
            b.set_facecolor((.85, .85, .85))
            b.set_edgecolor((0, 0, 0))
    ...
    plt.tight_layout(pad=0)
    plt.savefig("results_with_mtrie.pdf")
