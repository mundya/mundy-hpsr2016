...
This key assignation scheme, \textit{XYP}, is common in SpiNNaker applications.
...
The minimized table exploits TCAM ordering to ensure that all routing keys match the correct entries.
...
\section{Ordered-Covering}
  ...
  However, \mytt{00XX} intersects with \mytt{0000}.
  Moving \mytt{0000} below \mytt{00XX} by including it in the merge would change the behaviour of the routing table, hence \mytt{0000} may not be included in the merge.
  ...
  \begin{codebox}
    \Procname{$\proc{Refine-Upcheck}(\id{merge}, \id{table})$}
    \li \For $e$ \kw{in} \id{merge}
        \Do
        \li \For $e_\mathrm{other}$ \kw{in} $\id{table}(e\twodots\id{merge}]$
        \Do
    \li \If $\proc{Intersect}(e, e_\mathrm{other})$
        \Then
    \li $\id{merge} \gets \id{merge} - e$
    \li \kw{break}
        \End
        \End
        \End
  ...
  Once we have identified that the entry resulting from a merge, $e_m$ (e.g., \mytt{11XX}), intersects with an entry lower in the table, $e_c$ (e.g., \mytt{1110}), we attempt to modify the merge to remove the intersection.
  ...
  In our example we look to convert either of the \mytt{X}s in \mytt{11XX} such that the new entry resulting from the merge, $e_m^\prime$, is either \mytt{11\underline{0}X} or \mytt{11X\underline{1}} -- neither of which intersect with \mytt{1110}, $e_c$.
  ...
  We repeat the above process until no entries are covered (that is $e_m$ does not intersect with any $e_c$) and a valid merge is produced or it is not possible to modify the merge to avoid intersecting a lower entry and the merge must be abandoned.
...
% Centroid: 19152 bytes of which 14328 bytes is the table
  ...
  Other structures, such as sets of entries being considered as merge candidates can be implemented using bit-vectors and consume very little memory -- e.g., \SI{256}{\byte} for a 2048-entry table.
  ...
  If the space saved by minimizing the routing table could be reclaimed (by using a bucket allocator, for example) then any merge of more than ten entries will reduce the memory required to represent the routing and the aliases tables.

  Our implementation of Ordered-Covering does not currently reclaim the memory saved by reducing the size of a routing table.
  However, we found that the peak heap usage of our minimizer when running on SpiNNaker was \SI{17.9}{\kibi\byte} for the locally-connected benchmark (of which \SI{13.34}{\kibi\byte} was used to represent the routing table) and \SI{18.7}{\kibi\byte} for the centroid model (of which \SI{14.0}{\kibi\byte} was required for the routing table).
