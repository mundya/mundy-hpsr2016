...
We introduce Ordered-Covering (OC), a memory-efficient routing table minimizer which, subject to a heuristic which sorts the routing table in increasing number of \mytt{X}s and two rules which ensure the correctness of the final table, greedily merges entries with equivalent routes.
We show that parallel use of OC is able to significantly decrease the time required to minimize the routing tables in our benchmarks while using at most \SI{4.9}{\kibi\byte} more memory than is required to represent the routing table.
...
An extensively annotated and `production-ready' Python implementation of the Ordered-Covering algorithm is available as part of the `Rig' SpiNNaker library: \url{https://goo.gl/uhTCpa}
The C implementation of Ordered-Covering, used in our experiments, is also available online (\url{https://goo.gl/qM228T}) and may be compiled for either PC or SpiNNaker.
