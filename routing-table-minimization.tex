...
We show that, for one benchmark, on-chip routing table minimization using OC results in a 66.0$\times$ speed-up compared with performing the minimization off-chip.
For a second, more challenging, benchmark we show that a 1.9$\times$ speed-up in table minimization time is achieved by combined on- and off-chip minimization.
  \end{abstract}
