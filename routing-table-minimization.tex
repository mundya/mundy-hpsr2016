...
\section{Ordered-Covering}
  ...
  \noindent Our initial merge candidate, the first three entries, would become \mytt{11XX} and would be inserted above the bottom entry \emph{covering} \mytt{1110}, one of the entries' aliases, preventing it from being routed correctly.
  We describe a simple iterative algorithm for refining the merge to eliminate the covering below.
  
  To resolve this ambiguity we must modify our merge by removing entries such that some \mytt{X} bits in the merged entry become \emph{fixed} to either \mytt{0} or \mytt{1}.
  We first identify the groups of \mytt{X} bits which must be fixed to remove ambiguity as those which are \mytt{X}s in the merged entry but not in the covered entry.
  In this example, either fixing the right-most bit as \mytt{1} (\mytt{11X1}) or fixing the next right-most bit as \mytt{0} (\mytt{110X}) is sufficient to disambiguate the merged entry from the covered entry.
  
  For each group of fixable bits we produce a new merge candidate with entries removed such that the required bits are fixed.
  For example, to fix the right-most bit as \mytt{1} we must remove \mytt{110\underline{0}} and \mytt{111\underline{X}} from the merge candidate since the right-most bits of these entries cannot be merged into an entry with \mytt{1} on the right.
  To fix the next right-most bit as \mytt{0} we must remove \mytt{11\underline{1}X} since this cannot be merged into an entry with a \mytt{0} in that bit.
  
  The new merge candidate with the fewest entries removed is then selected.
  In our example, we remove \mytt{111X} from the merge candidate resulting in a new candidate:
  ...
  We now repeat the above process until either no entries are covered and a valid merge is produced.
  If an ambiguity arises which cannot be resolved by removing entries from the merge the algorithm terminates and no merge is produced.
  
  In the example above, the new merge candidate results in an entry \mytt{110X} being inserted between the bottom two entries.
  Since this merge does not cover any other entries in the table, the algorithm terminates.
  
  Note that in general some covered entries may require more than one bit to be fixed to disambiguate them.
  It is likely that resolving ambiguities requiring may fixed bits will lead to many entries being removed from the merge candidate.
  As an optimisation to reduce the number of merges candidates considered, we only repair the ambiguities with the fewest bits to be fixed in each iteration.
