...
\renewbibmacro{finentry}{%
  \iffieldequalstr{entrykey}{Davies2012}%<- key after which you want the break
  {\finentry\newpage}
{\finentry}}
\bibliography{paper}
...
We show that, for one benchmark, on-chip routing table minimization using OC results in a \SI{66.0}{\times} speed-up compared with performing the minimization off-chip.
For a second, more challenging, benchmark we show that a \SI{1.9}{\times} speed-up in table minimization time is achieved by combined on- and off-chip minimization.
  \end{abstract}
...
Packets are routed around this network by Address Event Representation (AER)~\parencite{Boahen2000}; each packet contains a \emph{routing key} which identifies its source and routers forward (and may duplicate) packets towards their destination(s).
...
While this algorithm has been shown to effectively exploit `default routing' to reduce the size of routing tables, the routing tables of both our benchmarks were too large to fit in the number of entries available, even when entries which could be replaced by default routing were removed (see \figurename~\ref{fig:results/final}).
...
\section{Ordered-Covering}
  ...
  When merging routing table entries, the following \textit{Ordered-Covering rules} are used to ensure the behaviour of the table remains consistent:
  ...
  \end{description}
    ...
    This is only a partial ordering of the table, and as an additional heuristic we require that new entries of generality $g$ be inserted \emph{below} existing entries of generality $g$.
  ...
  Doing so would mean that \mytt{0000} would be combined into the new entry \mytt{XXX0}, positioned below \mytt{00XX}, covering \mytt{0000}.
  ...
  Consequently, merging the first three entries as indicated is not allowed as it violates the down-check rule, changing the behaviour of the routing table.
...
\end{figure*}
  ...
  While this is not true of the centroid benchmark, OC is able to reduce \SI{48.6}{\percent} of the tables to fewer than 1024 entries.
...
% Centroid: 19152 bytes of which 14328 bytes is the table
  ...
  Consequently, on-chip OC represents a \SI{66.0}{\times} speed-up when compared to using order-exploiting Espresso.
  ...
  In this instance combined use of OC and Espresso represents a \SI{1.9}{\times} speed-up when compared to just using order-exploiting Espresso.
...
% This approach may be valuable in cases where default routing is sufficient since these routes are very cheap to compute.
  ...
  Using Ordered-Covering, in parallel on SpiNNaker, we were able to minimize all the tables in one of our benchmarks in \SI{13.6}{\second} -- a speed-up of \SI{66.0}{\times} compared to off-chip minimization using Espresso.
  In the other benchmark, combined use of on- and off-chip minimization using Ordered-Covering and Espresso was able to achieve a speed-up of \SI{1.9}{\times} of the time taken by off-chip minimization alone.
