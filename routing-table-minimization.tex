...
For a second, more challenging, benchmark we show that a 46.6\% reduction in routing table minimization time is achieved by combined on- and off-chip minimization.
  \end{abstract}
...
\section{Ordered-Covering}
  ...
  When minimizing a routing table, the following \textit{Ordered-Covering rules} are used to ensure the behaviour of the table remains consistent.
  These rules forbid any merge which would:

  \begin{description}[\IEEEsetlabelwidth{``Down-check''}]
    \item[``Up-check'']
      cause any entry in the merge to become \textit{covered} by an entry higher up the table.
    ...
    \item[``Down-check'']
      cause any \textit{aliased entry} below the merge to become \textit{covered},
  ...
  The \textit{down-check} ensures that the entry resulting from a merge would not match a subset of the packets expected to match entries below the point where the entry resulting from the merge would be located.
  ...
  We prioritize the uncovering of entries for which fewer bits may be converted.
